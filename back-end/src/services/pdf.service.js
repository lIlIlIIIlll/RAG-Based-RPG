  ...
  for (const paragraph of paragraphs) {
    ...
    if (paragraphWords > targetWords * 1.5) {
      ...
      for (const sentence of sentences) {
        ...
        if (
          currentWordCount + sentenceWords > targetWords &&
          currentChunk.trim()
        ) {
          chunks.push(currentChunk.trim());
      ...
      if (
        currentWordCount + paragraphWords > targetWords &&
        currentChunk.trim()
      ) {
        chunks.push(currentChunk.trim());
...
async function vectorizePDF(
  ...
  if (!text || text.trim().length < 50) {
    throw new Error(
      "PDF não contém texto extraível suficiente. O documento pode ser escaneado/imagem.",
    );
  }

  console.log(
    `[PDF Service] Texto extraído: ${text.length} caracteres, ${numPages} páginas.`,
  );
  ...
  console.log(
    `[PDF Service] Vetorização concluída: ${processedCount}/${chunks.length} chunks salvos.`,
  );
...
async function listVectorizedDocuments(chatToken, collectionName) {
  try {
    const allRecords = await lanceDBService.getAllRecordsFromCollection(
      chatToken,
      collectionName,
    );
  ...
  try {
    ...
    for (const record of allRecords) {
      if (record._documentId === documentId) {
        await lanceDBService.deleteRecordByMessageId(
          chatToken,
          record.messageid,
        );
        deletedCount++;
    ...
    console.log(
      `[PDF Service] Documento ${documentId} removido: ${deletedCount} chunks deletados.`,
    );
    return deletedCount;
  } catch (error) {
