...
function calculateSessionId(chatMetadata, newTextTokens) {
  ...
  if (projectedTokens > TOKEN_LIMIT_PER_SESSION) {
    ...
    const currentNum =
      parseInt(currentSession.id.replace("session_", ""), 10) || 1;
    const newSessionId = `session_${currentNum + 1}`;

    console.log(
      `[Session] Limite de ${TOKEN_LIMIT_PER_SESSION} tokens atingido. Criando nova sessão: ${newSessionId}`,
    );
...
async function getSessionMemories(chatToken, sessionId) {
  ...
  const history = await lanceDBService.getAllRecordsFromCollection(
    chatToken,
    "historico",
  );
  ...
  console.log(
    `[Session] Carregadas ${sessionMessages.length} memórias da ${sessionId} (${tokenCount} tokens)`,
  );
...
function loadPendingQueue() {
  try {
    if (fs.existsSync(PENDING_QUEUE_PATH)) {
      ...
      console.log(
        `[EmbeddingQueue] Carregadas ${items.length} inserções pendentes do disco.`,
      );
    }
  } catch (e) {
    console.error(
      "[EmbeddingQueue] Erro ao carregar fila persistente:",
      e.message,
    );
  }
}
...
function savePendingQueue() {
  try {
    ...
    fs.writeFileSync(
      PENDING_QUEUE_PATH,
      JSON.stringify(pendingEmbeddingsQueue, null, 2),
    );
  } catch (e) {
    console.error(
      "[EmbeddingQueue] Erro ao salvar fila persistente:",
      e.message,
    );
  }
}
...
function addToPendingQueue(item) {
  ...
  savePendingQueue();
  console.log(
    `[EmbeddingQueue] Adicionado à fila: ${item.messageid} (${pendingEmbeddingsQueue.length} na fila)`,
  );
...
async function processEmbeddingQueue() {
  ...
  console.log(
    `[EmbeddingQueue] Processando ${pendingEmbeddingsQueue.length} itens pendentes...`,
  );
  ...
  for (let i = 0; i < pendingEmbeddingsQueue.length; i++) {
    ...
    try {
      ...
      const vector = await geminiService.generateEmbedding(
        item.text,
        item.apiKeys,
      );
      ...
      const isZeroVector =
        !vector || vector.reduce((a, b) => a + Math.abs(b), 0) < 0.001;
      if (isZeroVector) {
      ...
      if (wasUpdated) {
        console.log(
          `[EmbeddingQueue] ✓ Embedding atualizado: ${item.messageid}`,
        );
        ...
        console.warn(
          `[EmbeddingQueue] Registro não encontrado: ${item.messageid}`,
        );
        itemsToRemove.push(i);
      }
      ...
      if (item.retryCount >= MAX_RETRIES) {
        console.error(
          `[EmbeddingQueue] ✗ Máximo de retries atingido para ${item.messageid}: ${error.message}`,
        );
        itemsToRemove.push(i);
        failCount++;
        ...
        console.log(
          `[EmbeddingQueue] Todas as keys em cooldown. Pausando processamento.`,
        );
        break;
      } else {
        console.warn(
          `[EmbeddingQueue] Retry ${item.retryCount}/${MAX_RETRIES} para ${item.messageid}: ${error.message}`,
        );
      }
    }
  }
  ...
  console.log(
    `[EmbeddingQueue] Processamento concluído: ${successCount} sucesso, ${failCount} falhas, ${pendingEmbeddingsQueue.length} restantes`,
  );
...
async function createChat(userId) {
  ...
  console.log(
    `[Service] Criando novo chat com token: ${chatToken} para user: ${userId}`,
  );
...
async function getChatHistory(chatToken) {
  ...
  return await lanceDBService.getAllRecordsFromCollection(
    chatToken,
    "historico",
  );
}
...
async function addMessage(
  ...
  if (apiKey && attachments.length > 0) {
    for (const att of attachments) {
      ...
      if (
        ...
        try {
          console.log(
            `[Service] Gerando descrição RAG para anexo: ${att.name} (${att.mimeType})`,
          );
          const description = await geminiService.describeMediaForRAG(
            att.data,
            att.mimeType,
            apiKey,
          );
          ...
          console.error(
            `[Service] Falha ao gerar descrição para ${att.name}: `,
            error.message,
          );
  ...
  if (mediaDescriptions.length > 0) {
    ...
    console.log(
      `[Service] Embedding enriquecido com ${mediaDescriptions.length} descrição(ões) de mídia.`,
    );
  }
  ...
  if (apiKey && textForEmbedding.trim().length > 0) {
    try {
      ...
      console.error(
        "[Service] Falha ao gerar embedding para mensagem:",
        error.message,
      );

      // Para coleções críticas (fatos, conceitos), podemos lançar erro
      const isCriticalCollection =
        collectionName === "fatos" || collectionName === "conceitos";

      if (failOnEmbeddingError && isCriticalCollection) {
        // FAIL-FAST: Lança erro para o chamador tratar
        const err = new Error(
          `Falha ao gerar embedding: ${error.message}. A memória não será buscável até que o embedding seja gerado.`,
        );
  ...
  if (embeddingStatus === "pending" && textForEmbedding.trim().length > 0) {
    ...
    console.warn(
      `[Service] Memória ${messageid} inserida com vetor zerado.Será reprocessada em background.`,
    );
  }
...
async function editMessage(chatToken, messageid, newText) {
  ...
  if (wasUpdated) {
    ...
    console.warn(
      `[Service] Falha ao editar mensagem ${messageid} (nﾃ｣o encontrada ?).`,
    );
  }
...
async function handleChatGeneration(
  ...
  if (!googleApiKeys || googleApiKeys.length === 0) {
    throw new Error(
      "API Key do Google não configurada (necessária para embeddings).",
    );
  }
    ...
    (async () => {
      try {
        ...
        if (result.repaired > 0) {
          console.log(
            `[Service] AUTO - REPAIR: ${result.repaired} embeddings reparados em background.`,
          );
        }
      } catch (err) {
  ...
  const totalAttachmentSize = files.reduce(
    (sum, f) => sum + f.buffer.length,
    0,
  );
  if (totalAttachmentSize > MAX_INLINE_SIZE_BYTES) {
    throw new Error(
      `Arquivos anexados excedem o limite de 20MB(${(totalAttachmentSize / 1024 / 1024).toFixed(2)}MB enviados).`,
    );
  }

  // 2.1 Calcula sessionId baseado nos tokens da mensagem
  const userMessageTokens = estimateTokens(userMessage);
  const { sessionId: currentSessionId, updatedMetadata } = calculateSessionId(
    chatMetadata,
    userMessageTokens,
  );

  // Salva metadados atualizados (atualiza contadores de sessão)
  await chatStorage.saveChatMetadata(
    chatToken,
    updatedMetadata,
    chatMetadata.userId,
  );
  ...
  console.log(
    `[Session] Mensagem do usuário (${userMessageTokens} tokens) atribuída à ${currentSessionId}`,
  );
  ...
  const historyRecords = await lanceDBService.getAllRecordsFromCollection(
    chatToken,
    "historico",
  );
  // Ordena por data
  ...
  // Define cli2apiOptions antecipadamente (usado em várias partes do fluxo)
  // Se o provider for cli2api, usa o process manager para obter a instância dedicada do usuário
  let cli2apiOptions;
  if (useCli2apiProvider) {
    if (!chatMetadata.userId) {
      throw new Error(
        "Este chat não está associado a um usuário. Crie um novo chat para usar o CLI2API.",
      );
    }
    try {
      const processInfo = await processManager.ensureProcess(
        chatMetadata.userId,
      );
      cli2apiOptions = {
        baseUrl: processInfo.baseUrl,
        apiKey: processInfo.apiKey,
      };
    } catch (error) {
      throw new Error(
        `Falha ao iniciar CLI2API para o usuário: ${error.message}. Verifique se o binário está acessível.`,
      );
    }
  } else {
    // Fallback para providers que usam cli2api internamente (search, summaries)
    cli2apiOptions = {
  ...
  // === GERAÇÃO DE RESUMO DE SESSÃO FECHADA ===
  // Se uma nova sessão foi criada, gera resumo da sessão anterior em background
  if (updatedMetadata.needsNewSession) {
    const closedSessionId = `session_${parseInt(currentSessionId.replace("session_", ""), 10) - 1 || 1}`;
    const closedSessionMessages = historyRecords.filter(
      (r) => r.sessionId === closedSessionId,
    );

    if (closedSessionMessages.length > 0) {
      // Gera em background para não bloquear a resposta
      (async () => {
        try {
          const closedSessionText = closedSessionMessages
            .map((r) => `${r.role}: ${r.text}`)
            .join("\n");

          const sessionSummary = await cli2apiService.generateSessionSummary(
            closedSessionText,
            closedSessionId,
            cli2apiOptions,
          );

          // Salva o resumo nos metadados
          if (updatedMetadata.sessions[closedSessionId]) {
            updatedMetadata.sessions[closedSessionId].summary = sessionSummary;
            await chatStorage.saveChatMetadata(
              chatToken,
              updatedMetadata,
              chatMetadata.userId,
            ...
            console.log(
              `[Service] Resumo da sessão ${closedSessionId} salvo nos metadados.`,
            );
          }
        } catch (error) {
          console.warn(
            `[Service] Falha ao gerar resumo da sessão fechada: ${error.message}`,
          );
        }
      })();
    }
  ...
  // === PREVIOUSLY ON: Resumo das mensagens anteriores da sessão atual ===
  // Filtra mensagens da sessão atual que não estão no recentHistory
  const currentSessionMessages = historyRecords.filter(
    (r) => r.sessionId === currentSessionId,
  );
  const previouslyOnStartIdx = 0;
  const previouslyOnEndIdx =
    currentSessionMessages.length - RECENT_HISTORY_LIMIT;
  let previouslyOnText = "";

  if (previouslyOnEndIdx > 0) {
    // Há mensagens anteriores na sessão que não estão na janela de contexto
    const previouslyOnMessages = currentSessionMessages.slice(
      previouslyOnStartIdx,
      previouslyOnEndIdx,
    );
    const previouslyOnMessagesText = previouslyOnMessages
      .map((r) => `${r.role}: ${r.text}`)
      .join("\n");

    try {
      previouslyOnText = await cli2apiService.generatePreviouslyOnSummary(
        previouslyOnMessagesText,
        cli2apiOptions,
      );
      console.log(
        `[Service] Previously On gerado para ${previouslyOnMessages.length} mensagens anteriores.`,
      );
    } catch (error) {
      console.warn(`[Service] Falha ao gerar Previously On: ${error.message}`);
    }
  }

  // 4. Gera Query de Busca usando LLM (Gemini 3 Pro via cli2api)
  // Formata o histórico completo para o LLM analisar
  const historyTextForQuery = recentHistory
    .map((r) => `${r.role}: ${r.text}`)
    .join("\n");

  // Recupera memórias da busca anterior (se houver) do cache/state
  // Por enquanto, usamos string vazia na primeira busca de cada turno
  const previousMemoriesText = clientVectorMemory || "";

  // cli2apiOptions já definido anteriormente

  // Gera query otimizada usando Gemini 3 Pro
  let searchQueryText;
  try {
    searchQueryText = await cli2apiService.generateSearchQuery(
      historyTextForQuery,
      previousMemoriesText,
      cli2apiOptions,
    );
    console.log(
      `[Service] Query RAG gerada via LLM (${searchQueryText.length} chars)`,
    );
  } catch (error) {
    // Fallback: usa últimas mensagens concatenadas
    console.warn(
      `[Service] Falha ao gerar query via LLM: ${error.message}. Usando fallback.`,
    );
    const QUERY_MESSAGE_COUNT = 6;
    const queryStartIndex = Math.max(
      0,
      historyRecords.length - QUERY_MESSAGE_COUNT,
    );
    const queryMessages = historyRecords.slice(queryStartIndex);
    searchQueryText = queryMessages.map((r) => r.text).join("\n");
  }

  // 5. Recupera Memoria (RAG) usando a query simplificada
  // Busca em TODAS as colecoes, ordena por relevancia e limita por palavras
  let contextText = "";
  let displayMemory = [];
  let uniqueResults = []; // Para uso nas tools

  // Fallback para garantir que collectionNames exista
  const collectionsToSearch = config.collectionNames || [
    "historico",
    "fatos",
    "conceitos",
  ];
  ...
  // === BUSCA UNICA USANDO TEXTO DAS ULTIMAS MENSAGENS ===
  if (searchQueryText && searchQueryText.trim().length > 0) {
    for (const collectionName of collectionsToSearch) {
      try {
        // Busca candidatos para filtrar depois
  ...
  console.log(
    `[Service] Busca concluida: ${allMemories.length} resultados encontrados`,
  );

  if (allMemories.length > 0) {
    // === SESSION-BASED DECAY ===
    // Memórias de sessões anteriores recebem penalidade proporcional à distância
    const currentSessionNum =
      parseInt(chatMetadata.currentSession.id.replace("session_", ""), 10) || 1;
    const SESSION_DECAY_RATE = 0.05; // 5% de penalidade por sessão de distância
    const SESSION_DECAY_MAX = 0.5; // Máximo 50% de penalidade

    // === BIAS ADAPTATIVO PARA FATOS E CONCEITOS ===
    const RELEVANCE_THRESHOLD = 0.7;
    const MAX_BOOST = 0.62;
    const HISTORICO_PENALTY = 1.016;

    allMemories.forEach((memory) => {
      // Guarda distância original para debug
      memory._originalDistance = memory._distance;

      // Aplica decay baseado em sessão
      const memorySessionNum =
        parseInt(
          (memory.sessionId || "session_1").replace("session_", ""),
          10,
        ) || 1;
      const sessionGap = currentSessionNum - memorySessionNum;

      if (sessionGap > 0) {
        const sessionDecay = Math.min(
          sessionGap * SESSION_DECAY_RATE,
          SESSION_DECAY_MAX,
        );
        memory._distance *= 1 + sessionDecay;
        memory._sessionDecay = sessionDecay;
      }

      // Bias adaptativo por categoria
      if (memory.category === "historico") {
        memory._distance = memory._distance * HISTORICO_PENALTY;
      } else if (
        memory.category === "fatos" ||
        memory.category === "conceitos"
      ) {
        if (memory._distance < RELEVANCE_THRESHOLD) {
          const relevanceFactor = Math.pow(
            1 - memory._distance / RELEVANCE_THRESHOLD,
            2,
          );
    ...
    // Conjunto de IDs ja presentes no historico recente para evitar duplicacao
    const recentHistoryIds = new Set(recentHistory.map((r) => r.messageid));
    const seenIds = new Set();

    // === SELECAO SIMPLES POR DISTANCIA ===
    const WORD_LIMIT = 5000;
    let currentWordCount = 0;

    // Preenche ate o limite de palavras (ordenado por distancia - mais relevantes primeiro)
    ...
    console.log(
      `[Service] Memorias selecionadas: ${uniqueResults.length} resultados (~${currentWordCount} palavras)`,
    );
    ...
    if (uniqueResults.length > 0) {
        ...
        if (m.attachments) {
          try {
            ...
            for (const att of atts) {
              ...
              if (
                ragMediaParts.length < MAX_RAG_IMAGES &&
                att.mimeType &&
                (att.mimeType.startsWith("image/") ||
                  att.mimeType === "application/pdf")
              ) {
      ...
      contextText =
        "Memﾃｳrias Relevantes recuperadas do banco de dados:\n" +
        memoryLines.join("\n");
      ...
      if (ragMediaParts.length > 0) {
        ...
        console.log(
          `[Service] ${ragMediaParts.length} mﾃｭdia(s) recuperada(s) do RAG para injeﾃｧﾃ｣o no contexto.`,
        );
      }
      ...
      console.log(
        `[Service] Contexto RAG construﾃｭdo com ${uniqueResults.length} memﾃｳrias(~${currentWordCount} palavras).`,
      );
    }
  }
  ...
  if (uniqueResults._ragMediaParts && uniqueResults._ragMediaParts.length > 0) {
    ...
    console.log(
      `[Service] ${ragMediaParts.length} mﾃｭdia(s) do RAG injetada(s) no histﾃｳrico na posiﾃｧﾃ｣o ${insertPosition}.`,
    );
  }
  ...
  if (
    ...
    console.warn(
      "[Service] Histﾃｳrico comeﾃｧa com 'model', inserindo placeholder 'user'.",
    );
  ...
  // 7. System Instruction Dinamico (injeta contexto sintetizado + Previously On)
  ...
  if (contextText && contextText.length > 0) {
    try {
      ...
      console.warn(
        `[Service] Falha na síntese de contexto: ${error.message}. Usando memórias brutas.`,
      );
      synthesizedContext = contextText;
    }
  }

  // === INJETA PREVIOUSLY ON ===
  // Adiciona resumo das mensagens anteriores da sessão atual
  if (previouslyOnText && previouslyOnText.length > 0) {
...
<retrieved_memory>`,
    ...
    console.log(
      `[Service] Previously On injetado nas system instructions (${previouslyOnText.length} chars).`,
    );
  }

  // === INJETA MEMÓRIAS SINTETIZADAS ===
  if (synthesizedContext) {
    if (finalSystemInstruction.includes("{vector_memory}")) {
      ...
      // Fallback para templates antigos que nao tenham a tag
      ...
      function_declarations: [
        {
          name: "insert_fact",
          description:
            "Salva um fato importante, regra de mundo ou acontecimento na memﾃｳria de longo prazo (coleﾃｧﾃ｣o 'fatos'). Use isso para coisas concretas que aconteceram.",
          parameters: {
            type: "OBJECT",
            properties: {
              text: {
                type: "STRING",
                description: "O conteﾃｺdo do fato a ser memorizado.",
              },
            },
            required: ["text"],
          },
        },
        {
          name: "insert_concept",
          description:
            "Salva um conceito, explicaﾃｧﾃ｣o abstrata, traﾃｧo de personalidade ou lore na memﾃｳria de longo prazo (coleﾃｧﾃ｣o 'conceitos').",
          parameters: {
            type: "OBJECT",
            properties: {
              text: {
                type: "STRING",
                description: "O conteﾃｺdo do conceito a ser memorizado.",
              },
            },
            required: ["text"],
          },
        },
        {
          name: "roll_dice",
          description:
            "Realiza uma rolagem de dados de RPG (ex: 1d20, 2d6+3, 4dF). Retorna o resultado.",
          parameters: {
            type: "OBJECT",
            properties: {
              ...
              type: {
                type: "STRING",
                description: "Tipo do dado (20, 6, 100, F para Fudge/Fate).",
              },
              modifier: {
                type: "INTEGER",
                description: "Modificador a ser somado ao total (opcional).",
              },
            },
            required: ["count", "type"],
          },
        },
        {
          name: "edit_memory",
          description:
            "Edita o texto de uma memﾃｳria existente (fato ou conceito) ou mensagem do histﾃｳrico. Use quando o usuﾃ｡rio corrigir uma informaﾃｧﾃ｣o ou quando um fato mudar.",
          parameters: {
            type: "OBJECT",
            properties: {
              messageid: {
                type: "STRING",
                description: "O ID da mensagem/memﾃｳria a ser editada.",
              },
              new_text: {
                type: "STRING",
                description: "O novo texto atualizado.",
              },
            },
        ...
        },
        {
          ...
          description:
            "Remove memﾃｳrias (fatos ou conceitos) ou mensagens que nﾃ｣o sﾃ｣o mais verdadeiras ou relevantes. Use APENAS quando tiver certeza que a informaﾃｧﾃ｣o deve ser esquecida.",
          parameters: {
            type: "OBJECT",
            properties: {
              messageids: {
                ...
                items: { type: "STRING" },
                description:
                  "Lista de IDs das mensagens/memﾃｳrias a serem deletadas.",
              },
            },
            required: ["messageids"],
          },
  ...
  const generateResponse = async (history, systemInst, options) => {
    if (useGoogleProvider) {
      return await googleProvider.generateChatResponse(
        history,
        systemInst,
        options,
      );
    } else if (useCerebrasProvider) {
      return await cerebrasService.generateChatResponse(
        history,
        systemInst,
        options,
      );
    } else if (useCli2apiProvider) {
      return await cli2apiService.generateChatResponse(
        history,
        systemInst,
        options,
      );
    }
    return await openrouterService.generateChatResponse(
      history,
      systemInst,
      options,
    );
  ...
  while (loopCount < 5) {
    ...
    const hasValidTextWithTools =
      text && text.trim().length > 10 && !text.trim().startsWith("...");
    if (hasValidTextWithTools && !finalModelResponseText) {
      console.log(
        `[Service] Modelo retornou texto junto com tools.Capturando narraﾃｧﾃ｣o: "${text.substring(0, 50)}..."`,
      );
      finalModelResponseText = text;
    }
      ...
      try {
        if (name === "insert_fact") {
          ...
          if (memoryInsertCount > 0) {
            console.log(
              `[Service] Aguardando ${MEMORY_INSERT_DELAY}ms antes de inserir memﾃｳria(rate limit protection)...`,
            );
          ...
          toolResult = {
            status: "success",
            message: "Fato inserido com sucesso.",
          };
          ...
          toolResult = {
            status: "success",
            message: "Conceito inserido com sucesso.",
          };
          ...
          const rollMsgId = await addMessage(
            chatToken,
            "historico",
            resultText,
            "model",
          ...
          const wasUpdated = await editMessage(
            chatToken,
            args.messageid,
            args.new_text,
          );
          if (wasUpdated) {
            toolResult = {
              status: "success",
              message: "Memﾃｳria atualizada com sucesso.",
            };
          } else {
            toolResult = {
              status: "error",
              message: "Memﾃｳria nﾃ｣o encontrada ou erro ao atualizar.",
            };
          }
          ...
          for (const id of ids) {
            ...
            if (memory) {
              ...
              memoriesToDelete.push({
                messageid: id,
                text: "(Memﾃｳria nﾃ｣o encontrada no contexto recente)",
                category: "?",
              });
            }
          }
          ...
          toolResult = {
            ...
            message:
              "Aguardando confirmaﾃｧﾃ｣o do usuﾃ｡rio para deletar memﾃｳrias.",
    ...
    if (finalModelResponseText && !needsFollowUp) {
      console.log(
        `[Service] Texto jﾃ｡ capturado, pulando requisiﾃｧﾃ｣o adicional.`,
      );
      break;
    }
    ...
    currentResponse = await generateResponse(
      conversationHistory,
      finalSystemInstruction,
      generationOptions,
    );
  ...
  if (
    !isValidContent(finalModelResponseText) &&
    currentResponse &&
    isValidContent(currentResponse.text)
  ) {
  ...
  if (!isValidContent(finalModelResponseText)) {
    console.log(
      "[Service] Resposta final vazia ou placeholder, tentando forﾃｧar resposta de texto...",
    );
    try {
      const finalAttempt = await generateResponse(
        conversationHistory,
        finalSystemInstruction,
        {
          ...generationOptions,
  ...
  const modelResponse = isValidContent(finalModelResponseText)
    ? finalModelResponseText
    : "Desculpe, nﾃ｣o consegui processar sua solicitaﾃｧﾃ｣o.";
  ...
  const modelMessageId = await addMessage(
    chatToken,
    "historico",
    modelResponse,
    "model",
  ...
  const finalHistory = await lanceDBService.getAllRecordsFromCollection(
    chatToken,
    "historico",
  );
...
async function importChat(userId, messages, apiKey, onProgress) {
  console.log(
    `[Service] Importando chat para user: ${userId} com ${messages.length} mensagens.`,
  );
  ...
  for (const msg of messages) {
    ...
    const { sessionId, updatedMetadata } = calculateSessionId(
      chatMetadata,
      messageTokens,
    );
...
async function branchChat(originalChatToken, targetMessageId, userId) {
  console.log(
    `[Service] Criando branch do chat ${originalChatToken} a partir da mensagem ${targetMessageId} `,
  );
  ...
  const fullHistory = await lanceDBService.getAllRecordsFromCollection(
    originalChatToken,
    "historico",
  );
  const targetMessage = fullHistory.find(
  ...
  console.log(
    `[Service] Cutoff time definido: ${cutoffTime} (Msg ID: ${targetMessageId})`,
  );
  ...
  for (const collectionName of config.collectionNames) {
    const originalRecords = await lanceDBService.getAllRecordsFromCollection(
      originalChatToken,
      collectionName,
    );
    ...
    console.log(
      `[Service] Copiando ${filteredRecords.length} registros da coleﾃｧﾃ｣o '${collectionName}' para o novo chat.`,
    );
    ...
    for (const record of filteredRecords) {
      ...
      await lanceDBService.insertRecord(
        newChatToken,
        collectionName,
        cleanRecord,
      );
...
async function exportMemories(chatToken, collections = ["fatos", "conceitos"]) {
  console.log(
    `[Service] Exportando memﾃｳrias do chat ${chatToken}.Coleﾃｧﾃｵes: ${collections.join(", ")} `,
  );
  ...
  for (const collectionName of collections) {
    try {
      const records = await lanceDBService.getAllRecordsFromCollection(
        chatToken,
        collectionName,
      );
      ...
      console.log(
        `[Service] Exportados ${exportedRecords.length} registros de '${collectionName}'(com embeddings).`,
      );
    } catch (err) {
      console.warn(
        `[Service] Erro ao exportar coleﾃｧﾃ｣o '${collectionName}': `,
        err.message,
      );
...
async function importMemories(chatToken, data, collections, onProgress) {
  console.log(
    `[Service] Importando memﾃｳrias para chat ${chatToken}.Coleﾃｧﾃｵes: ${collections.join(", ")} `,
  );
  ...
  const hasEmbeddings =
    data.version === "1.1" &&
    data.embeddingDimension === config.embeddingDimension;

  if (hasEmbeddings) {
    console.log(
      `[Service] Arquivo contﾃｩm embeddings compatﾃｭveis(${data.embeddingDimension}D).Importaﾃｧﾃ｣o rﾃ｡pida ativada.`,
    );
  } else {
    console.log(
      `[Service] Arquivo sem embeddings ou incompatﾃｭvel.Serﾃ｡ necessﾃ｡rio gerar embeddings.`,
    );
    if (!apiKey) {
      throw new Error(
        "API Key do Google nﾃ｣o configurada (necessﾃ｡ria para gerar embeddings).",
      );
    }
  }
  ...
  for (const collectionName of collections) {
    if (!data.collections || !data.collections[collectionName]) {
      console.warn(
        `[Service] Coleﾃｧﾃ｣o '${collectionName}' nﾃ｣o encontrada no arquivo.`,
      );
      continue;
    }
    ...
    for (const record of records) {
      try {
        ...
        const hasValidVector =
          ...
          record.vector.length === config.embeddingDimension;
    ...
    console.log(
      `[Service] Importados ${stats.imported[collectionName]} registros para '${collectionName}'.`,
    );
  }

  console.log(
    `[Service] Importaﾃｧﾃ｣o concluﾃｭda.Embeddings reutilizados: ${stats.embeddingsReused}, Gerados: ${stats.embeddingsGenerated} `,
  );
...
async function searchAllUserChats(userId, queryText, apiKey) {
  console.log(
    `[Service] Iniciando busca global para user ${userId}: "${queryText.substring(0, 30)}..."`,
  );
  ...
  console.log(
    `[Service] Busca global concluﾃｭda em ${elapsed} ms.${rankedChats.length} chats encontrados.`,
  );
...
async function repairEmbeddings(chatToken) {
  ...
  console.log(
    `[Service] Iniciando reparo de embeddings para chat ${chatToken}...`,
  );
...
async function checkZeroEmbeddings(chatToken) {
  return await lanceDBService.countZeroEmbeddings(chatToken, [
    "conceitos",
    "fatos",
    "historico",
  ]);
