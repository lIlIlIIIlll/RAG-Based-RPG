...
const CEREBRAS_MODELS = [
    ...
    { id: "zai-glm-4.6", name: "Z.ai GLM 4.6 (Reasoning)", speed: "~1000 tokens/s" },
    { id: "zai-glm-4.7", name: "Z.ai GLM 4.7 (Reasoning)", speed: "~1000 tokens/s" },
];
...
async function generateChatResponse(
    ...
    return retryOperation(async () => {
        ...
        if (modelName.includes('glm')) {
            ...
            requestBody.reasoning_effort = generationOptions.reasoningEffort || 'medium';
        }
        ...
        if (!response.ok) {
            ...
            if (
                ...
                if (!response.ok) {
                    ...
                    throw new Error(`Cerebras API error (${response.status}): ${retryErrorData}`);
                }
            } else {
                ...
                const error = new Error(`Cerebras API error (${response.status}): ${errorData}`);
                ...
                if (response.status === 429) {
                    ...
                    error.userMessage = "Erro de autenticação com a Cerebras. Verifique sua API Key.";
                } else {
        ...
        if (reasoning) {
            console.log(`[Cerebras] Reasoning tokens detectados (${reasoning.length} chars).`);
        }
