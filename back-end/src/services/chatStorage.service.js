...
if (!fsSync.existsSync(METADATA_DIR)) {
  ...
  console.log(
    `[SISTEMA] | [STORAGE] -> Diretório de metadados criado em: ${METADATA_DIR}`,
  );
}
...
async function saveChatMetadata(chatToken, data, userId = null) {
  ...
  try {
    ...
    console.log(
      `[${new Date().toLocaleTimeString()}] | [STORAGE] -> Metadados salvos para o chat: ${chatToken} (User: ${userId})`,
    );
  } catch (error) {
    console.error(
      `[${new Date().toLocaleTimeString()}] | [STORAGE] -> ERRO ao salvar metadados:`,
      error,
    );
    throw error;
  }
...
async function getChatMetadata(chatToken) {
  ...
  try {
    ...
    if (error.code === "ENOENT") {
      console.warn(
        `[${new Date().toLocaleTimeString()}] | [STORAGE] -> Chat não encontrado: ${chatToken}`,
      );
      return null;
    }
    console.error(
      `[${new Date().toLocaleTimeString()}] | [STORAGE] -> ERRO ao ler chat:`,
      error,
    );
    throw error;
  }
...
async function getAllChats(userId) {
  try {
    ...
    console.error(
      `[${new Date().toLocaleTimeString()}] | [STORAGE] -> ERRO ao listar chats:`,
      error,
    );
    return [];
  }
...
async function updateChatConfig(chatToken, newConfig) {
  ...
  console.log(
    `[${new Date().toLocaleTimeString()}] | [STORAGE] -> Configurações atualizadas para: ${chatToken}`,
  );
  return currentData;
}
...
async function deleteChatMetadata(chatToken) {
  ...
  try {
    ...
    console.log(
      `[${new Date().toLocaleTimeString()}] | [STORAGE] -> Arquivo de metadados deletado: ${chatToken}`,
    );
  } catch (error) {
    if (error.code !== "ENOENT") {
      console.error(
        `[${new Date().toLocaleTimeString()}] | [STORAGE] -> ERRO ao deletar arquivo:`,
        error,
      );
      throw error;
    }
...
async function updateChatTitle(chatToken, newTitle) {
  ...
  await saveChatMetadata(chatToken, currentData, currentData.userId);
  console.log(
    `[${new Date().toLocaleTimeString()}] | [STORAGE] -> Título atualizado para: ${chatToken}`,
  );
  return currentData;
}
